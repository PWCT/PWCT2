...
#Parameters
gNbrSteps = 1
#For Points in Shape Step BY  gNbrSteps
gGoAroundFigure = 1
#Draw Shape X times
#----------------------------
#DFT - Test Pattern
...
counter = 1
### Modulo 3 to change Pen Color
###--------------
colorGreen = new qcolor()
{
	setrgb(000,255,000,255)
}
colorRed = new qcolor()
{
	setrgb(255,000,000,255)
}
colorBlue = new qcolor()
{
	setrgb(000,000,255,255)
}
colorBlack = new qcolor()
{
	setrgb(000,000,000,255)
}
colorYellow = new qcolor()
{
	setrgb(255,255,000,255)
}
### Yellow
colorWhite = new qcolor()
{
	...
	win = new qWidget()
	{
		...
		Canvas = new qlabel(win)
		{
			...
			daVinci = new qpainter()
			{
				begin(MonaLisa)
				translate(200,300)
				### TRANSLATE COORDINATE x,y   0,0    MAPS TO 300,300
...
func pPress  { 
	...
	counter++
	daVinci.setCompositionMode(26)
	###  THIS is the MAGIC - Erase Old Line when Redrawn
	oldX = initX
	### Save Pressed point as OLD points
	oldY = initY
	return 
	###------------------------------------------------------------------------
} 
...
func pMove  { 
	win.setWindowTitle("Press: Top "+initX+"-"+initY+" Bottom "+finalX+"-"+finalY+" Width "+(finalX-initX)+" Height "+(finalY-initY))
	### Screen coordinate, NOT Canvas getx()  gety()
	finalX = myfilter.getglobalx()
	finalY = myfilter.getglobaly()
	pDrawObjects()
	Canvas.setpixmap(MonaLisa)
...
func WhereAreWe () { 
	...
	WinLeft = Rec.left()+8
	### <<< QT FIX because of Win Title
	WinTop = Rec.top()+30
	### <<< QT FIX because of Win Title
	WinRight = Rec.right()
	WinBottom = Rec.bottom()
	win {
		setwindowtitle("Window ReSize: Win "+WinWidth+"x"+WinHeight+" --- LT "+WinLeft+"-"+WinTop+" --- RB "+WinRight+"-"+WinBottom)
	}
	return 
	###---------------------------------------
	### Window Moved
...
func WhereMoved () { 
	...
	### <<< QT FIX because of Win Title
	win {
		setwindowtitle("Window Moved: xPos: "+xPos+" yPos: "+yPos)
	}
	return 
	###===============================================
	###===============================================
...
func pDrawObjectsXX () { 
	#Draw Image as is
	for i = 1 to len(trainPoints)-1 step 1 { 
...
func pDrawObjects () { 
	...
	if ShapeType = "Shape-PI" { 
		ShapePoints = ShapePI
		#Array of X-Y for PI Shape
		elseif ShapeType = "Shape-Circle"
			ShapePoints = ShapeCircle
		elseif ShapeType = "Shape-Train"
			ShapePoints = ShapeTrain
			gNbrSteps = 7
			#Fast processing
		elseif ShapeType = "Shape-Treble"
			ShapePoints = ShapeTreble
	} 
	oVertQ = new Queue
	oHorzQ = new Queue
	nbrPoints = len(ShapePoints)
	### Nbr of Points in the trainPoints for PI Shape
	#Discrete Fourier Transform call for the List of X-Y Points
	for loopAoundFigure = 1 to gGoAroundFigure step 1 { 
		YY = []
		XX = []
		nbrPoints = 0
		#===<<<<
		...
		FourierY = DFT(YY)
		dt = TWO_PI/nbrPoints
		### Increment angle amount
		#dt    = 0.051927                        ### Fixed value for test --- 2*PI / nbrPoints = 121
		#-------------------------------------------------------------------
		#Go Around each Circle = TWO_PI or 360 degrees
		### Angle is Radians, Inc by 1 trainPoints, till reach 2*PI = Full Circle
		for angle = 0 to TWO_PI step dt { 
			MonaLisa.fill(colorBlack)
			### Clear screen
			xV = 1
			yV = 1
			...
			horz = 500
			vert = 150
			#Draw Horizontal circle offsets
			piH = 500
			piV = 200
			#Draw PI offsets
			#----------------------------
			#EpiCycles - LEFT Vertical Circles
			for i = 1 to nbrPoints step 1 { 
				oldXV = xV
				oldYV = yV
				freq = fourierY[i][3]
				radius = fourierY[i][4]
				...
				daVinci.drawLine(oldXV,-oldYV+piV,XV,-YV+piV)
				#Draw Line - oldCircleCtr to newCircleCtr
				daVinci.setpen(curPen)
				DrawCircle(oldXV,-oldYV+piV,radius)
				#Draw multiple Vertical Circles
			...
			for i = 1 to nbrPoints step 1 { 
				...
				radius = fourierX[i][4]
				phase = fourierX[i][5]
				xH += radius*cos(freq*angle+phase)
				#+ HALF_PI)
				yH += radius*sin(freq*angle+phase)
				#+ HALF_PI )
				daVinci.setpen(penYellow)
				daVinci.drawLine(oldXH+horz,-oldYH-vert,XH+horz,-YH-vert)
				#Draw Line - oldCircleCtr to newCircleCtr
				daVinci.setpen(curPen)
				DrawCircle(oldXH+horz,-oldYH-vert,radius)
				#Draw multiple Horizontal Circles
			} 
			#------------------------------------------------------------------
			#VertWave Queue - History - Add modified y-Point to end of Queue
			#QUEUE - FirstIn-FirstOut,  Add to LastIn
			oVertQ.add(yV)
			#1st Set of Circles on Left for vertical
			oHorzQ.add(xH)
			#2nd Set of Circles on Right for horizontal
			lastPos = len(oVertQ)
			#-------------------------
			#Draw PI Shape
			daVinci.setpen(penYellow)
			...
			daVinci.DrawLine(xV,-yV+piV,oHorzQ[lastPos]+horz,-oVertQ[lastPos]+piV)
			#Draw Horz Line in Circles to Wave
			daVinci.DrawLine(xH+horz,-yH-vert,oHorzQ[lastPos]+horz,-oVertQ[lastPos]+piV)
			#Draw Vert Line in Circles to Wave
			daVinci.setpen(curPen)
			if lastPos > nbrPoints { 
				#QUEUE - Remove Start point
				oVertQ.remove()
				oHorzQ.remove()
			} 
			#Animated Delay
...
func DrawCircle (x,y,radius) { 
	...
	daVinci.drawEllipse(HCorner,VCorner,HWidth,VHeight)
	#LeftCorner, HWidth, VHeight
	return 
	#------------------------------------------
...
func DrawCirclePoint (x,y,radius,angle) { 
	...
	XPos = x*radius
	YPos = y*radius
	daVinci.drawPoint(HCenter+XPos,VCenter-YPos)
	#Minus Y goes up screen
	return [XPos, YPos]
	#------------------------------------------
...
func DrawCircleLine (x,y,radius,Angle) { 
	...
	XPos = x*radius
	YPos = y*radius
	daVinci.drawLine(HCenter,VCenter,HCenter+XPos,VCenter-YPos)
	#Minus Y goes up screen
	return [XPos, YPos]
	#------------------------------------------
	#------------------------------------------
...
func eventSliderNbrSteps () { 
	...
	gNbrSteps = sliderNbrSteps.value()
	gNbrSteps = ceil(gNbrSteps/10)
	#100 / 10 = 10 max
	if gNbrSteps = 0 { 
		gNbrSteps = 1
	} 
...
func eventSliderGoAroundFigure () { 
	#Max 100/10 = 10  Min 10 / 10 = 1
	gGoAroundFigure = sliderGoAroundFigure.value()
...
func DFT (x) { 
	BigN = len(x)
	BigX = list(BigN)
	#The Wave Array output - make it same size of input
	for k = 1 to BigN step 1 { 
		re = 0
		im = 0
		for n = 1 to BigN step 1 { 
			phi = (TWO_PI*k*n)/BigN
			re += x[n]*cos(phi)
			im -= x[n]*sin(phi)
		} 
