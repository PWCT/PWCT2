...
func main          		
	...
	oGame = New Game      	
	{
		...
		NewButton(oGame,20,500,200,80,"Left",func oGame,oSelf {
			function = oGame.find(:map).keypress
			call function(oGame,oGame.find(:map),KEY_LEFT)
		})
		NewButton(oGame,580,500,200,80,"Right",func oGame,oSelf {
			function = oGame.find(:map).keypress
			call function(oGame,oGame.find(:map),KEY_RIGHT)
		})

		NewButton(oGame,300,485,200,50,"Up",func oGame,oSelf {
			function = oGame.find(:map).keypress
			call function(oGame,oGame.find(:map),KEY_UP)
		})
		NewButton(oGame,300,545,200,50,"Down",func oGame,oSelf {
			function = oGame.find(:map).keypress
			call function(oGame,oGame.find(:map),KEY_DOWN)
		})
	...
	oGame {
		Object {
			...
			EventCode = cFunc
					...
					gl_draw_text(oFont,gl_map_rgb(0,0,0),x+width/2,y+(height/2)-15,1,Text)
				}
			}
