class GameCamera
	...
	func GameLevelCameraView  { 
		...
		glLoadIdentity()
		gluLookAt(x,10,z+5,x+lx,1.0,z+lz,0.0,1.0,0.0)
		if lPlay2D { 
	...
	func GameMapCameraView  { 
		...
		glViewport(SCREEN_W-C_MAP_SIZE_WIDTH,SCREEN_H-C_MAP_SIZE_HEIGHT-20,C_MAP_SIZE_WIDTH,C_MAP_SIZE_HEIGHT)
		glMatrixMode(GL_PROJECTION)
		glLoadIdentity()
	...
	func GameLevel2DCameraView  { 
		...
		glViewport(0,0,C_MAP_SIZE_WIDTH,C_MAP_SIZE_HEIGHT)
		glMatrixMode(GL_PROJECTION)
		glLoadIdentity()
	...
	func FastMoveLeft  { 
		for t = 1 to nSpeedRightLeft step 1 { 
			moveleft()
		} 
	} 
	func FastMoveRight  { 
	...
	func FastMoveUp  { 
		if lSlowMotion { 
			nToMoveUp += nSpeedUpDown
			return 
		} 
		for t = 1 to nSpeedUpDown step 1 { 
			moveup()
		} 
	} 
	func FastMoveDown  { 
		if lSlowMotion { 
			nToMoveDown += nSpeedUpDown
			return 
		} 
		for t = 1 to nSpeedUpDown step 1 { 
			movedown()
