...
class ReadBinaryController from windowsControllerParent
	...
	pageSize = 512
	func Choose_File  { 
		oView.Label1.SetText("0")
		oView.Label2.SetText("0")
		oView.Label3.SetText("0")
		frm = oView.win
		 new fileDialog(frm)
		{
			cName = getOpenFileName(frm,"open file","","All Files(*.*)")
		}
		if cName = "" { 
		...
		for x = 1 to pages step 1 { 
			oView.ListWidgetP.addItem("Page "+x)
		} 
	...
	func ShowPage pNum { 
		...
		oView.ListWidget1 {
			setViewMode(1)
			setSpacing(2)
			SetFont( new qfont("Consolas",14,-1,0))
		}
		oView.ListWidget2 {
			setViewMode(1)
			setSpacing(2)
