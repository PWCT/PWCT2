...
#Load OBJ model
texture = LoadTexture("plane_diffuse.png")
#Set map diffuse texture
resultMT = SetModelMaterialTexture(model,0,MAP_DIFFUSE,texture)
GenTextureMipmaps(texture)
#Define the camera to look into our 3d world
camera = Camera3D(0,60,-120,0,12,0,0,1,0,30,CAMERA_PERSPECTIVE)
pitch = 0.0
roll = 0.0
yaw = 0.0
SetTargetFPS(60)
...
while (!WindowShouldClose()) { 
	...
	DrawTexturePro(texBackground,Rectangle(0,0,texBackground.width,texBackground.height),Rectangle(centerX,centerY,texBackground.width*scaleFactor,texBackground.height*scaleFactor),Vector2(texBackground.width/2*scaleFactor,texBackground.height/2*scaleFactor+pitchOffset*scaleFactor),roll,WHITE)
	DrawTexturePro(texPitch,Rectangle(0,0,texPitch.width,texPitch.height),Rectangle(centerX,centerY,texPitch.width*scaleFactor,texPitch.height*scaleFactor),Vector2(texPitch.width/2*scaleFactor,texPitch.height/2*scaleFactor+pitchOffset*scaleFactor),roll,WHITE)
	...
	EndTextureMode()
	BeginMode3D(camera)
	DrawModel(model,Vector3(0,6.0,0),1.0,WHITE)
	...
	DrawTextureRec(framebuffer.texture,Rectangle(0,0,framebuffer.texture.width,-framebuffer.texture.height),Vector2(screenWidth-framebuffer.texture.width-20,20),Fade(WHITE,0.8))
