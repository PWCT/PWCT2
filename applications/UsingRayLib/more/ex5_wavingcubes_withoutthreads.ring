...
camera = Camera3D(16,14,16,0,0,0,0,1,0,45,0)
...
while !WindowShouldClose() { 
	...
	BeginMode3D(camera)
	DrawGrid(10,5.0)
	for x = 0 to nMax step 1 { 
		for y = 0 to nMax step 1 { 
			for z = 0 to nMax step 1 { 
				blockScale = (x+y+z)/30
				scatter = sin(blockScale*20+(time*4))
				Vec3Set(cubepos,(x-halfBlocks)*(scale*3)+scatter,(y-halfBlocks)*(scale*2)+scatter,(z-halfBlocks)*(scale*3)+scatter)
				cubeSize = (2.4-scale)*blockScale
				DrawCube_2(cubePos,cubeSize,cubeSize,cubeSize,colorFromHSV_2(((x+y+z)*18.0)%360,0.75,0.9))
			} 
		} 
	} 
	EndMode3D()
