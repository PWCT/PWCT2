...
#include "ring/src/ring_generallib_extension.c"
#include "ring/src/ring_math_extension.c"
#include "ring/src/ring_file_extension.c"
#include "ring/src/ring_general.c"
#include "ring/src/ring_state.c"
#include "ring/src/ring_refmeta_extension.c"
#include "ring/src/ring_list_extension.c"
#include "ring/src/ring_os_extension.c"
...
int main(int argc, char *argv[])
{
	...
	ringapp_deleteappfiles();
    	...
    	// Copy Ring Object File (colorappController.ringo) from Resources to Temp Folder
    	QString path2 ;
    	path2 = path+"/colorappController.ringo";
    	QFile::copy(":/colorappController.ringo",path2);
    	ring_state_runobjectfile(pRingState,"colorappController.ringo");
	...
	QFile oObjectFile(":/colorappController.ringo");
	...
	unsigned int nFileSize = (unsigned int) oObjectFile.size();
        ...
        ring_state_runobjectstring(pRingState,(char *) cCode,"colorappController.ringo");
        free(cCode);
...
void ringapp_deleteappfiles(void)
{
    ...
    path = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) ;
    QDir::setCurrent(path);
    ringapp_delete_file(path,"colorappController.ringo");
}
