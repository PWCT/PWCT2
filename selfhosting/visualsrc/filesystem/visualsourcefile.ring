...
class VisualSourceFile
	...
	func LoadStepsTable  { 
		...
		for record in aResult step 1 { 
			aStepsTable+[0+record[:stepid],0+record[:parentid],[:name = record[:name],:active = 0+record[:active],:code = record[:code],:interactionid = 0+record[:interactionid],:visible = 0+record[:visible],:stepnumber = 0+record[:stepnumber],:steptype = 0+record[:steptype],:plainname = record[:plainname]]]
		} 
	...
	func LoadInteractionsTable  { 
		...
		Log("Start - Query result to HashTable")
		for record in aResult step 1 { 
			aInteractionsTable+[0+record[:interactionid],0+record[:type],record[:component],record[:date],record[:time],record[:variablesvalues]]
		} 
		Log("End - Query result to HashTable")
		#/*
	...
	func SaveStepsTable  { 
		oDatabase.ExecBegin()
		for record in aStepsTable step 1 { 
			cSQL = "INSERT INTO STEPSTREE (STEPID,PARENTID,NAME,"+"ACTIVE,CODE,INTERACTIONID,VISIBLE,STEPNUMBER,STEPTYPE,PLAINNAME)"+"VALUES (#{V1},#{V2},'#{V3}',#{V4},'#{V5}', #{V6},#{V7},#{V8},#{V9},'#{V10}'); "+NL
	...
	func SaveInteractionsTable  { 
		oDatabase.ExecBegin()
		for record in aInteractionsTable step 1 { 
