...
class ProgramController
	cFileName = C_FILENONAME
	#"noname.ring"
	cSourceCode = ""
	cCurrentDir = CurrentDir()+"/"
	#The Goal Designer Folder
	if isWindows() { 
	...
	func RunOnMobile oGD { 
		...
		if PWCTIsMobile(:MobileAppLibs) { 
			cCode = 'load "mobileapplibs.ring"'+nl+'load "mobileapplibs2.ring"'+nl+'load "mobileapplibs3.ring"'+nl
		} 
...
private
	...
	func isBatchFile cFile { 
		if right(lower(trim(cFile)),4) = ".bat" OR right(lower(trim(cFile)),3) = ".sh" { 
			return True
		} 
		return False
	} 
