...
class GoalDesignerView from WindowsViewParent
	...
	win = new QMainWindow()
	{
		...
		oPageDesign = new qWidget()
		{
			oStepsTree = new StepsTreeView(oPageDesign)
			{
				...
				setFocusPolicy(2)
				#Support Keyboard keys like 'a' and 'z'
		...
		if  NOT PWCTIsMobile(:TimeMachineLabel) { 
			labelTM = new qLabel(win)
			{
				setText(T_GD_THETIMEMACHINE)
			}
			#"The Time Machine"
		} 
		btnPlay = new qPushButton(win)
		{
			#"Play As Movie"
		...
		win {
			...
			if T_LAYOUTDIRECTION { 
				tool1 = addtoolbar_2(Qt_RightToolBarArea,gdtool)
				else
	...
	func StepsTreeStyle  { 
		cStepsTreeStyle = "QTreeWidget { selection-color:"+C_STEPSTREE_SELECTIONCOLOR+"; selection-background-color:"+C_STEPSTREE_SELECTIONBACKGROUND+"; background-color: "+C_STEPSTREE_BACKCOLOR+"; }"
	...
	func StepsTreeWhiteAndBlackStyle nFontSize { 
		return "QTreeWidget { color: black; font-size: " + nFontSize+"pt  } " +"QTreeWidget::!active { selection-background-color:rgba(0,65,255,255) ; selection-color:white; } ; " 
	} 
