...
class TreeControl from qTreeWidget
	aTree = []
	#Node ID , Node Object , Node Object.pObject
	aTreeObjects = []
	#QWidget (Parent),QWidget (Parent).pObject
	font
	nFontSize = 12
	#The font object and the font size
	lUseLabels = False
	...
	oTempFont = NULL
	lActivateParent = True
	#Activate the parent step after creating the step
	#/*
	...
	func SerialAdd nParentID,nID,cText,lIgnore { 
		...
		oLabel.settext(NodeImage(cImage)+oStyle.text(cText,cColor,cBackColor))
	...
	func createTreeLabel  { 
		aTreeObjects+[ new TreeLabel(self)
		,NULL]
	...
	func InsertNode nParentID,nID,cText,nIndex { 
		...
		AddToTree(nID,oItem)
		setCurrentItem(oItem,0)
		#To Display the item (become visible)
		if lActivateParent { 
			setCurrentItem(oParent,0)
			#Focus on Parent Step
		} 
		if lUseLabels = True { 
			oLabel = createTreeLabel() {
				resize(100,30)
				...
				settext(this.NodeImage(C_LABELIMAGE_NODEICON)+this.oStyle.text(cText,this.cColor,this.cBackColor))
			}
			NewLabelStyle(oLabel,[:id = nID])
			setItemWidget(oItem,0,oLabel)
		} 
	...
	func SetLabelFont oLabel { 
		...
		if PWCTIsMobile(:FontFamily) { 
			oLabel.setStyleSheet("font-size:"+this.nFontSize+"pt;")
				...
				oLabel.setStyleSheet('font-family: "'+cFontFam+'" ; font-size:'+this.nFontSize+"pt;")
		} 
	} 
	...
	func AddNodesFromBuffer aNodesObjectsList,aNodesDataList { 
		for x = 1 to len(aNodesObjectsList) step 1 { 
