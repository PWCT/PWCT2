...
class StepsTreeView from TreeControl
	...
	lUseLabels = True
	#Use TreeLabel for each Tree Item
	aLabels = []
	#List of Labels controls	
	...
	func AddStartPoint  { 
		...
		oLabel = createTreeLabel() {
			resize(100,30)
			settext(this.NodeImage(C_LABELIMAGE_NODEICON)+this.oStyle.text(this.cStartPointText,C_STEPCOLOR_STARTPOINT,""))
		...
		if lUseLabels = false { 
			oItem.setText(0,cText)
				...
				if lIgnoreStatus { 
					...
					oLabel.SetText(cImage+this.oStyle.text(cText,this.cColor,this.cBackColor))
					else
	...
	func SubStepsList aList,oParentStep { 
		aList+oParentStep
		for x = 1 to oParentStep.childcount() step 1 { 
	...
	func SetStepColor nStepType { 
		switch nStepType { 
			...
			case C_STEPTYPE_ALLOWINTERACTION
				...
				#"black"
				cBackColor = C_STEPCOLOR_ALLOWINTERACTION_BACKCOLOR
				#"yellow"
	...
	func BlockStyleNotSupported  { 
		return not ( C_ENV_DEFAULT_STYLE= C_ENV_STYLE_WINDOWSB or C_ENV_DEFAULT_STYLE= C_ENV_STYLE_WHITEB or C_ENV_DEFAULT_STYLE= C_ENV_STYLE_BLACKB or C_ENV_DEFAULT_STYLE= C_ENV_STYLE_SIMPLEBLACK or C_ENV_DEFAULT_STYLE= C_ENV_STYLE_BLUE or C_ENV_DEFAULT_STYLE= C_ENV_STYLE_BLUE2 ) 
	} 
	func GoalDesigner  { 
