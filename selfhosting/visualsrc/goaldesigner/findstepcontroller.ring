...
class FindStepController from WindowsControllerParent
	...
	aStepIDResult = []
	#Array contains the IDs of the search result
	lSaveHistory = True
	...
	func ItemSelectedAction  { 
		...
		try { 
			...
			{
				setwindowtitle(T_GD_FS_SORRY)
				#"Sorry"
				settext(T_GD_FD_CANTFINDTHEITEM)
				#"Can't find the item"
	...
	func ReplaceAllAction  { 
		...
		parent().saveHistory()
		lSaveHistory = False
		for nStepID in aStepIDResult step 1 { 
			ReplaceStep(nStepID)
		} 
		lSaveHistory = True
		#/*
