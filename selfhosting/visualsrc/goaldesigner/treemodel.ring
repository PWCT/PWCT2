...
class TreeModel
	...
	#	*/
	aList = []
	#Tree Content [nNodeID - nParentID - Content]
	nID = 0
	#Automatic ID for each node
	aBuffer = []
	#List used for Cut, Copy & Paste operations
	#/*
	...
	func AddNode nParent,Content { 
		nID++
		#Increase the Automatic ID by 1
	...
	func FindNewNodePosition nParent { 
		...
		if len(aList) > 0 { 
			if aList[len(aList)][C_TREEMODEL_PARENTID] = nParent OR aList[len(aList)][C_TREEMODEL_NODEID] = nParent { 
				return len( aList ) 
			} 
		} 
		for x = 1 to len(aList) step 1 { 
			if aList[x][C_TREEMODEL_NODEID] = nParent { 
				aParentList = [nParent]
				for x2 = x+1 to len(aList) step 1 { 
	...
	func SiblingDown nNodeID { 
		...
		nPos2 = 0
		for x = nPos+1 to len(aList) step 1 { 
			if aList[x][C_TREEMODEL_PARENTID] = nParentID { 
				nPos2 = x
				exit 1
			} 
