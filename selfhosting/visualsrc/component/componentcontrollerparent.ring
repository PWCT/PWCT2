...
class ComponentControllerParent from WindowsControllerParent
	...
	nInteractionMode
	#New or Modify
	cVariablesValues = NULL
	#Variables Values
	nStepNumber = 0
	#Step Number
	lAllowInteraction = False
	#The step allow sub steps
	lStepIsAComment = False
	#The step is a comment
	lNoInteractionPage = False
	#No Interaction Page
	...
	aAllowEmptyValue = []
	#List of interaction page variables that could be empty
	#/*
	#		Add Comment
	#	*/
	...
	func CheckSyntax  { 
		for aItem in oView.aVariables step 1 { 
			if aItem[C_INTERACTION_VL_TYPE] = C_INTERACTION_CT_TEXTBOX OR aItem[C_INTERACTION_VL_TYPE] = C_INTERACTION_CT_EDITBOX { 
	...
	func common_callobjectmethod cFunc,cFuncName { 
		...
		NewStep(cOutputNameForStepName+cObjectNameForStepName+StyleData(cFuncName)+"("+StepData(:Value3)+")")
		SetStepCode(cOutputNameForStepCode+cObjectNameForStepCode+cFunc+"("+Variable(:Value3)+")")
		#/*
	...
	func SearchInListbox cVar { 
		...
		for t = 0 to oList.Count()-1 step 1 { 
	...
	func textDblClick cVariable { 
		...
		lastWindow().oView.win.resize(800,900)
