...
class ComponentViewParent from WindowsViewParent
	aVariables = []
	#List contains the [ Control Object , variable Name , Type ]
	oTitleLabel
	lFirstTextBox = True
	...
	win = new QWidget()
	{
		...
		setWindowTitle(T_IP_WINDOWTITLE)
		#"Interation Window"
	...
	func Title cTitle { 
		oTitleLabel = new qLabel(win)
		{
			...
			setStyleSheet("font-size:"+this.nTitleFontSize+"pt;color:"+C_INTERACTIONPAGE_TITLECOLOR+";background-color:"+C_INTERACTIONPAGE_TITLEBACKCOLOR+";")
	...
	func GetVariablesValues  { 
		...
		for item in aVariables step 1 { 
			...
			cVariablesValues += item[C_INTERACTION_VL_NAME]+C_INTERACTIONVALUES_SEPARATOR+cValue+C_INTERACTIONVALUES_SEPARATOR
		} 
	...
	func SetVariablesValues cVariablesValues { 
		...
		for x = 1 to len(aVariables) step 1 { 
			item = aVariables[x]
