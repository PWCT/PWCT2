...
class FormDesigner_QTimer from QLabel
	...
	func AddObjectProperties oDesigner { 
		...
		oDesigner.oView.AddProperty(T_FORMDESIGNER_ATTRIBUTE_INTERVAL,False)
		#"Interval"
		oDesigner.oView.AddProperty(T_FORMDESIGNER_ATTRIBUTE_TIMEOUTEVENT,False)
		#"Timeout Event"
	} 
	...
	func GenerateCode oDesigner { 
		cOutput = char(9)+char(9)+oDesigner.oModel.GetObjectName(self)+" = "+"new #{f1}(win) {
		#{f2}
				}"+nl
	...
	func GenerateCustomCode oDesigner { 
		cOutput = "setInterval(#{f1})"+nl+'setTimeoutevent("#{f2}")'+nl+"start()"+nl
