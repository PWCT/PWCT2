...
class FormDesigner_QTabWidget from QTabWidget
	...
	func SetPagesCountValue cValue { 
		...
		if nPagesCount > 0 { 
			for x = len(aTabs)+1 to nPagesCount step 1 { 
				aTabs+[ new qWidget()
				,"Page "+x]
				inserttab(len(aTabs)-1,aTabs[len(aTabs)][1],aTabs[len(aTabs)][2])
			} 
			if nPagesCount < len(aTabs) { 
				#Delete Tabs
	...
	func AddObjectProperties oDesigner { 
		...
		oDesigner.oView.AddProperty(T_FORMDESIGNER_ATTRIBUTE_PAGESCOUNT,False)
		#"Pages Count"
		oDesigner.oView.AddProperty(T_FORMDESIGNER_ATTRIBUTE_PAGESTITLESSCOMMA,False)
		#"Pages Titles (S: Comma)"
	} 
	...
	func GenerateCustomCode oDesigner { 
		cOutput = ""
		for x = 1 to len(aTabs) step 1 { 
			cOutput += "	#{f1}page#{f2} = new qwidget()"+nl+'inserttab(#{f3},#{f1}page#{f2},"#{f4}")'
			if  NOT x = len(aTabs) { 
				cOutput += nl
			} 
