...
class FormDesignerFileSystem from ObjectsParent
	...
	func GetFileNameFromDialog oDialog { 
		cInputFileName = ""
		if oDialog.selectedfiles().count() = 0 { 
			return 
		} 
	...
	func Objects2String oDesigner,aObjectsList { 
		...
		#Objects
		for x = 1 to len(aObjectsList) step 1 { 
			aObject = aObjectsList[x]
			cObjContent = Copy(char(9),1)+'[ :name = "#{f1}" , :id = #{f2} , :classname = "#{f3}" , :data = ['+nl
	...
	func DeleteAllobjects oDesigner { 
		for x = 2 to len(oDesigner.oModel.aObjectsList) step 1 { 
