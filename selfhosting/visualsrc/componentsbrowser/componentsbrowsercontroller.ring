...
class ComponentsBrowserController from WindowsControllerParent
	...
	#Write Ring source code (list) instead of using Visual Source File
	lWriteComponentsFile = False
	#Use Ring source code instead of using Visual Source File
	...
	func AddComponents  { 
		if !lUseComponentsFile { 
			...
			if isWebAssembly() { 
				oVisualSourceFile.cFileName = T_CB_COMPONENTSFILE
				#"vpl/components.pwct"
				elseif PWCTisMobile(:ComponentsFile)
					oVisualSourceFile.cFileName = CurrentDir()+"/"+T_CB_COMPONENTSFILE
					#"vpl/components.pwct"
				else
					oVisualSourceFile.cFileName = T_CB_COMPONENTSFILE
					#"vpl/components.pwct"
	...
	func TextualCodeToVisualCode cFind { 
		...
		for aStep in aStepsTree step 1 { 
	...
	func OpenSelectedByID nID { 
		...
		if !isClass(cFile+:ComponentController) { 
			return 
		} 
		#Start the Component
		if lInteractionPagesInGoalDesigner { 
			...
			parent().oView.widgetVPages.Show()
		...
		if (PWCTIsMobile(:ComponentSelected) AND parent().isDockForComponentsBrowser()) OR C_ENV_DEFAULT_MODE = C_ENV_MODE_GD { 
			parent().parent().oView.oDockGoalDesigner.raise()
		} 
		parent().oView.win.raise()
	...
	func GetComponentByFileName cComponent { 
		for item in oModel.getData() step 1 { 
