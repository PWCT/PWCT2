...
class CallMethodComponentController from ComponentControllerParent
	...
	func GenerateAction  { 
		...
		NewStep(T_CT_CALLMETHOD_ST_CALLMETHOD+cOutputEqual+cObject+Variable(:Name)+T_CT_CALLMETHOD_ST_CALLSTART+cParametersData+T_CT_CALLMETHOD_ST_CALLEND)
		SetStepCode(cOutputEqual+cObject+Variable(:Name)+"("+cParameters+")")
		return True
	} 
private
