...
class ComponentWizardGenerator
	...
	*/
	...
	func ProcessTemplate cTemplate { 
		...
		cWordsTemplate = substr(cTemplate,nPos)+substr(cTemplate,1,nPos)
		...
		nIndex = 1
		for word in aWordsEnglish step 1 { 
		...
		nIndex = 1
		for word in aWordsArabic step 1 { 
		...
		cEnglishFile = TemplateValues(cTemplateHeader)+WindowsNL()+list2str(aWordsEnglish)
		cArabicFile = TemplateValues(cTemplateHeader)+WindowsNL()+list2str(aWordsArabic)
